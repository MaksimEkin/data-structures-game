"""
python manage.py test game_board.database.tests_db
"""
...
class DBCreate(TestCase):
    ...
    def test_duplicatecreate(self):
        ...
        self.assertEqual( created_game, FAIL_PHRASE, msg=f'{BColors.FAIL}\t[-]\tGame was not created in the Database!{BColors.ENDC}')
...
class DBRead(TestCase):
    def test_read_correct(self):
        ...
        self.assertEqual( read_game["game_id"], board["game_id"], msg=f'{BColors.FAIL}\t[-]\tGame Id returned from read board does not equal expected!{BColors.ENDC}')
    ...
    def test_read_nonexist(self):
        ...
        self.assertEqual( read_game, FAIL_PHRASE, msg=f'{BColors.FAIL}\t[-]\tIncorrect return for non-existant game!{BColors.ENDC}')
...
class DBupdate(TestCase):
    def test_update_correct(self):
        ...
        mongo.create_game( board )
        updated_game = mongo.update_game( board["game_id"], board )
        mongo.remove_game( board["game_id"] )

        self.assertEqual( updated_game["game_id"], board["game_id"], msg=f'{BColors.FAIL}\t[-]\tGame Id returned from read board does not equal expected!{BColors.ENDC}')
    ...
    def test_update_nonexist(self):
        ...
        updated_game = mongo.update_game( "This name should really not exist in the database, and if it does, YEESH!", board )

        self.assertEqual( updated_game, FAIL_PHRASE, msg=f'{BColors.FAIL}\t[-]\tIncorrect return for non-existant game!{BColors.ENDC}')
...
class DBdelete(TestCase):
    def test_delete_correct(self):
        ...
        #Make sure the game exists first
        created_game = mongo.create_game( board )
    ...
    def test_delete_nonexist(self):
        ...
        self.assertEqual( deleted_game, FAIL_PHRASE, msg=f'{BColors.FAIL}\t[-]\tIncorrect return for non-existant game!{BColors.ENDC}')
